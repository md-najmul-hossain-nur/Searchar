...
if (empty($_SESSION['role']) || !in_array($_SESSION['role'], ['police', 'policeman'], true) || empty($_SESSION['user_id'])) {
...
try {
  $stmt = $pdo->prepare("SELECT police_id AS id, full_name, email, mobile, profile_photo, cover_photo, bio, date_of_birth, gender, street, city, country FROM policemen WHERE police_id = :id LIMIT 1");
...
try {
  ...
  if ($pdo->query("SHOW COLUMNS FROM posts LIKE 'status'")->fetch()) $hasStatus = true;
  if ($pdo->query("SHOW COLUMNS FROM posts LIKE 'media_json'")->fetch()) $hasMediaJson = true;
  if ($pdo->query("SHOW COLUMNS FROM posts LIKE 'share_anonymous'")->fetch()) $hasShareAnonymous = true;

  $selectCols = "id, author_name, text, media_path, media_type, category, created_at";
  if ($hasMediaJson) $selectCols .= ", media_json";
  if ($hasShareAnonymous) $selectCols .= ", share_anonymous";
  if ($hasStatus) $selectCols .= ", status";

  $statusWhere = $hasStatus ? " AND status = 'approved'" : '';
  $postStmt = $pdo->prepare("SELECT {$selectCols} FROM posts WHERE author_id = :author_id AND author_role IN ('police','policeman'){$statusWhere} ORDER BY id DESC LIMIT 50");
...
<body>
  ...
  <div class="cover-photo">
    <img src="<?= !empty($user['cover_photo']) ? '../uploads/police/' . e($user['cover_photo']) : '../Images/demo_pic/cover.jpg' ?>"
      alt="Cover" class="cover-img" onerror="this.onerror=null;this.src='../Images/default-cover.gif';">
    <div class="profile-pic-container">
      <img src="<?= !empty($user['profile_photo']) ? '../uploads/police/' . e($user['profile_photo']) : '../Images/demo_pic/profile.jpg' ?>"
        class="profile-pic" alt="Profile" onerror="this.onerror=null;this.src='../Images/demo_pic/profile.jpg';">
    </div>

    <div class="main-content">
      <div class="left-panel">
        <div class="card user-info" style="position: relative;">
          <button class="edit-btn" title="Profile Setting" onclick="location.href='../Html/Policeman_Edit_profile.php?police_id=<?= $user_id ?>'">
            <img src="../Images/settings.gif" alt="" aria-hidden="true"> Profile Setting
          </button>
      ...
      <div class="center-panel">
        <div class="card share-box">
          <img class="mini-profile" src="../Images/post.gif" alt="Post">
        ...
        <div id="post-feed">
          ...
          <?php else: ?>
            <?php foreach ($profilePosts as $post): ?>
              <?php
                $postAuthorName = (string)($post['author_name'] ?? ($user['full_name'] ?? 'Officer Name'));
              ...
              <div class="card post" data-post-id="<?= (int)($post['id'] ?? 0) ?>" data-share-anonymous="<?= $isAnonymous ? '1' : '0' ?>">
                <div class="post-header">
                  <img src="<?= e($displayAuthorPhoto) ?>" class="mini-profile" alt="Profile"
                    onerror="this.onerror=null;this.src='../Images/demo_pic/profile.jpg';">
                  <div>
        ...
        <div class="post-modal-content">
          ...
          <div class="facebook-toggle">
            <label class="facebook-toggle-switch">
              ...
              <span class="facebook-toggle-slider"><i class="fa-solid fa-user-secret"></i></span>
            </label>
      ...
      <div class="right-panel">
        ...
        <div class="notifications">
          <div class="redzone">
            ...
            <button class="redzone-btn" onclick="window.location.href='../Html/RedZone.html';">
              Open Red Zone Map
            </button>
          </div>
        </div>
  ...
  <style>
    ...
    .redzone h4 { margin-bottom: 10px; color: #c0392b; font-weight: 700; }
    .redzone ul { list-style: none; padding: 0; margin: 0 0 10px 0; }
    .redzone ul li {
      display: flex;
    ...
    .redzone ul li span:last-child { font-size: 12px; color: #888; }
    .redzone-btn {
      width: 100%;
      background: #e74c3c;
      color: white;
    ...
    .redzone-btn:hover { background: #c0392b; }
  </style>
